...
class WorkerServiceTest {
    ...
    @Test
    void list() {
        Worker worker = sut.list().list().stream().filter(
    ...
    @Test
    void findById() {
        Worker worker = sut.findById(new WorkerIdentifier(1L));
        assertAll(
                () -> assertEquals(worker.mailAddress().toString(), "fukawa_teruyoshi_new@example.com"),
                () -> assertEquals(worker.phoneNumber().toString(), "03-1234-9999"),
                () -> assertEquals(worker.name().toString(), "布川 光義"));
    ...
    void registerAndDelete() {
        ...
        Worker foundWorker = sut.findById(registeredWorker.identifier());
        assertAll(
