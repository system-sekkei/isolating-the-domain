...
class ActualWorkDateTimeTest {
    ...
    void 労働時間が週４０時間を超えていない場合の１日あたりの法定時間外労働時間を計算できる() {
        ...
        OverLegalHoursWorkTime overLegalHoursWorkTime = OverLegalHoursWorkTime.from(
            new MonthlyTimeRecord(new TimeRecords(list)),
            new BeforeMonthlyTimeRecord(new TimeRecords(List.of())),
            workDate);
